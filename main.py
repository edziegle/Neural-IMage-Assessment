...
def main(config):
    ...
    if config.warm_start:
        model.load_state_dict(
            torch.load(
                os.path.join(config.ckpt_path, "epoch-%d.pth" % config.warm_start_epoch)
            )
        )
    ...
    if config.train:
        ...
        for epoch in range(config.warm_start_epoch, config.epochs):
            ...
            print(
                "Epoch %d completed. Mean EMD loss on val set: %.4f."
                % (epoch + 1, avg_val_loss)
            )
            writer.add_scalars(
                "epoch losses",
                {"epoch train loss": avg_loss, "epoch val loss": avg_val_loss},
                epoch + 1,
            )
